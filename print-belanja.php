...
<body><br><br><br>
	...
	if(isset($_GET['filter']) && ! empty($_GET['filter'])){ // Cek apakah user telah memilih filter
		...
		if($filter == '1'){ // Jika filter nya 1 (per tanggal)
			...
			echo 'Ket : Laporan Belanja Tanggal '.$tgl.'<br /><br />';
		...
		?>
			...
			echo "Yth. Pimpinan Kusuma Jaya Batik<br>";
			echo "Hal : Laporan Keuangan<br>";
			echo 'Ket : Semua Data Belanja<br /><br />';

		$query = "SELECT * FROM belanja inner join user inner join sumber WHERE belanja.id_sumber = sumber.id_sumber AND sumber.id_sumber = belanja.id_sumber AND belanja.id_user = user.id_user AND user.id_user = belanja.id_user"; // Tampilkan semua data transaksi diurutkan berdasarkan tanggal
	}
	?>
	<table border="1" width="100px">
	<tr>
		<th style="text-align: center; width: 5px;">Tanggal</th>
		<th style="text-align: center; width: 5px;">Jumlah (Rp.)</th>
		<th style="text-align: center; width: 5px;">Oleh</th>
		<th style="text-align: center; width: 5px;">Sumber Dana</th>
		<th style="text-align: center; width: 5px;">Keterangan</th>
<!-- 		<th>Barang</th>
		<th>Jumlah</th>
		<th>Total Harga</th> -->
	</tr>
	...
	if($row > 0){ // Jika jumlah data lebih dari 0 (Berarti jika data ada)
		while($data = mysqli_fetch_array($sql)){ // Ambil semua data dari hasil eksekusi $sql
			...
			$tgl = date('d-m-Y', strtotime($data['tgl_pengeluaran']));
			if ($data['is_update']!=1) {
			echo "<tr>";
			// echo "<td>".$tgl."</td>";
 ...
 //        ?><br><?php
	?>
		...
		<br>
		<p>Laporan Keuangan ini dibuat dengan sesungguhnya untuk dipergunakan sebagaimana mestinya. Laporan ini dibuat oleh <?=$_SESSION['nama']?> pada tanggal <?php echo (new \DateTime())->format('d-m-Y');?> , dengan Total Belanja  Rp. <?php echo number_format($ttl,2,',','.') ?> ,- </p>
<br><br></div>
...
$pdf->WriteHTML($html);
$pdf->Output('Laporan Belanja.pdf', 'D');
?>
